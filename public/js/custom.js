var W3Crm = function () {
	...
	var handleMinHeight = function () {
		...
		if (win_h > 0 ? win_h : screen.height) {
			...
			// $(".content-body").css("min-height", "aut");
	...
	var theme = function () {
		...
		const applyTheme = (theme) => {
			...
			$('#theme i').attr('class', `fa-regular fa-${icon}`);
			$('#theme i').css('color', theme === 'dark' ? '#fff' : '#f0f0f0');
			$('#icon-theme-title i').css('color', theme === 'dark' ? '#fff' : '#000');
			// $('.menu-icon i').toggleClass('text-dark', theme ===  'dark' ? 'text-dark' : 'text-light');
